...
# ----------------------------
# Fallback Camera intrinsics (K guess)
# ----------------------------
...
# ----------------------------
# Load calibration (K, dist) from NPZ and scale K to current frame size
# ----------------------------
def load_and_scale_calibration(npz_path: str, target_w: int, target_h: int):
    """
    Reads K and dist from calibration_filtered.npz and scales K if needed
    to match (target_w, target_h).

    Your calibration_filtered.npz contains: K (3x3), dist (1x5)
    It does NOT contain image_size, so we infer old size approximately.
    """
    data = np.load(npz_path)
    K = data["K"].astype(np.float64)
    dist = data["dist"].astype(np.float64)

    # Infer original resolution from principal point (approx)
...
# ----------------------------
# Essential matrix pose (with undistortion)
# ----------------------------
def estimate_pose_from_points(K, dist, pts1, pts2, ransac_thresh=0.7):
    """
    We undistort points first using real calibration.
    """
    if dist is not None:
...
def main():
    ...
    parser.add_argument("--calib", default="calibration_filtered.npz",
                        help="NPZ calibration file containing K and dist")
    ...
    # ✅ Use calibration if exists, else fallback
    calib_path = Path(args.calib)
    if calib_path.exists():
        K, dist = load_and_scale_calibration(str(calib_path), w, h)
        print(f"✅ Using calibration from: {calib_path}")
    else:
        K = build_K_from_frame_size(w, h)
        dist = None
        print("⚠️ Calibration file not found -> using estimated K only")
    ...
    prev_aruco = aruco_camera_position(gray_prev, K, dist if dist is not None else np.zeros((5, 1)),
                                       marker_m, dict_name=args.aruco_dict)
    ...
    while True:
        ...
        # --- ArUco ground truth ---
        cur_aruco = aruco_camera_position(gray, K, dist if dist is not None else np.zeros((5, 1)),
                                          marker_m, dict_name=args.aruco_dict)
        ...
        try:
            R_rel, t_rel, inliers = estimate_pose_from_points(
                K, dist, pts1, pts2, ransac_thresh=0.7
            )
    ...
    traj_aruco = np.array(traj_aruco) if len(traj_aruco) > 0 else None
    ...
    print("K used:\n", K)
    if dist is not None:
        print("dist used:", dist.ravel())
    print("SIFT points:", len(traj_sift_smoothed))
    print("ArUco points:", 0 if traj_aruco is None else len(traj_aruco))
